...
static void cabecalho_venda(Pet *pet) {
    ...
    if (pet->nome != NULL)
        printf("  Pet: %-20s | ID: %-5d | Tutor ID: %d\n",
               pet->nome, pet->id, pet->id_Cliente);
    else
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
static int opcao_finalizar(Carrinho *carrinho, Pet *pet,
                           FILE *arq_produtos, FILE *arq_vendas)
{
    if (carrinho->qtd_itens == 0) {
        printf("\n  Carrinho vazio. Adicione pelo menos um produto antes de finalizar.\n");
        aguardar_enter_vet();
        return 0;
    }
...
void MenuVendaAtendimento(Pet *pet) {
    ...
    while (!venda_concluida) {
        ...
        switch (opcao) {
            ...
            case 4:
                venda_concluida = opcao_finalizar(&carrinho, pet,
                                                  arq_produtos, arq_vendas);
                break;

            case 0:
                if (carrinho.qtd_itens > 0) {
                    printf("\n  Cancelar a venda? O carrinho sera descartado. (1=Sim / 0=Nao): ");
                    int conf = ler_inteiro_vet();
                    if (conf == 1) venda_concluida = 1;
                } else {
                    venda_concluida = 1;
                }
                break;
    ...
    fclose(arq_produtos);
    if (arq_vendas != NULL) fclose(arq_vendas);
}
